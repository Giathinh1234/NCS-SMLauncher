...
def draw_visualizer(screen, player, w, h, mode, t):
    ...
    if mode == "radial":
        radius_base = min(w, h) * 0.22
        for i, m in enumerate(mag):
            ang = 2 * math.pi * i / NUM_BARS - math.pi / 2
            length = radius_base * 0.15 + float(m) * radius_base * 0.9
            col = neon_color(i / NUM_BARS + t * 0.05)
            x0 = cx + math.cos(ang) * radius_base
            y0 = cy + math.sin(ang) * radius_base
            x1 = cx + math.cos(ang) * (radius_base + length)
            y1 = cy + math.sin(ang) * (radius_base + length)
            pygame.draw.line(surf, col, (int(x0), int(y0)),
                             (int(x1), int(y1)), 4)
        beat = float(mag.mean())
        r_core = radius_base * 0.35 * (1 + beat * 0.25)
        core_col = neon_color(t * 0.05)
        pygame.draw.circle(surf, (*core_col, 60), (cx, cy), int(r_core))
        pygame.draw.circle(surf, (*core_col, 200), (cx, cy), int(r_core * 0.55))
    else:
...
def draw_ui(screen, font, font_big, tracks, selected, player, w, h, muted,
    ...
    panel_h = min(h - 140, 60 + len(tracks) * 26)
    ...
    visible_start = max(0, min(selected - 12, len(tracks) - 20))
    for row_i, ti in enumerate(range(visible_start,
                                     min(visible_start + 20, len(tracks)))):
...
def main():
    ...
    hover_idx = None
    base_y = h - 90
    hint_surf = font.render("↑↓ select  ⏎ play  space pause  ←→ seek  "
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mx, my = event.pos
                hover_idx = None
                if 24 <= mx <= 364 and 62 <= my <= 62 + 20 * 26:
                    row = (my - 66) // 26
                    visible_start = max(0, min(selected - 12, len(tracks) - 20))
                    ti = visible_start + row
                    if 0 <= row < 20 and 0 <= ti < len(tracks):
                        hover_idx = ti
